...
plot(ln_est ~ year, data=dat, type='l',
     ...
     main="Kuskokwim River Chinook Salmon: AR1-Base")
...
g <- resid.list %>% ggplot(aes(x=year, y=resid)) +
       ...
       ggtitle("Kuskokwim River Chinook Salmon: AR1-Base")
g
...
# Plot: Forecast =================================================
par(mfrow=c(3,1))
hist(pars$fcst/1e3, main="AR1-Base: Estimation Uncertainty in Point Estimate",
       xlab="2020 Predicted Run Size (thousands)", col='blue', 
       freq=FALSE, breaks=100)


hist(pars$post_fcst/1e3, main="AR1-Base: Posterior Predictive Distribution",
     xlab="2020 Predicted Run Size (thousands)", col='blue', 
     freq=FALSE, breaks=100)


hist(pars$post_fcst_unc/1e3, main="AR1-Base: Posterior Predictive Distribution (uncorrected)",
     xlab="2020 Predicted Run Size (thousands)", col='blue', 
     freq=FALSE, breaks=100)
...
plotPost(pars$post_fcst_unc/1e3, xlim=c(150,300))
...
plotPost(pars$post_fcst/1e3, xlim=x.lim,
         xlab="2020 Predicted Run Size (thousands)", ylab="Relative Probability",
         main="Kuskokwim River Chinook Salmon: AR1-Base", showCurve=TRUE)
dev.off()
